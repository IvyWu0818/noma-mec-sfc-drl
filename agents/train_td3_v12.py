...
from envs.iiot_env_v12 import IIoTEnvV12, TX_POWER_W, NOISE_POWER_W, BANDWIDTH_MHZ
...
class V12MetricsCallback(BaseCallback):
    """
    V12 新增追蹤:
      - episode_avg_sinr_db: SINR (dB) 分布，驗證 5G 17 dB 中距離場景
      - episode_avg_rate:    平均傳輸速率 (Mbps)，驗證 B×log₂(1+SINR) 合理性
    """
...
def main():
    env = Monitor(IIoTEnvV12(), "logs/v12_")
    ...
    print("=" * 60)
    print("V12 訓練啟動 (5G NR / NOMA 合規版本)")
    print("=" * 60)
    print(f"[RF 參數]")
    ...
    print(f"  t_ul (40Kb)  : {_t_ul_example:.2f} ms")
    print(f"[MEC 節點算力]")
    print(f"  mec0: 35 GHz, mec1: 45 GHz, mec2: 55 GHz (MCycles/ms)")
    print(f"[延遲預算] deadline 10–22 ms (5G IIoT 製程自動化, TR22.804)")
    print(f"[Obs=21, Action=16 | Total timesteps: 1,000,000]")
    print("=" * 60)

    model.learn(total_timesteps=1_000_000, callback=callback)
    model.save("models/td3_iiot_v12_final")

    output_path = "results/td3_v12_training_metrics.json"
    with open(output_path, "w") as f:
        json.dump(callback.metrics, f, indent=2)
    print(f"V12 數據已存至 {output_path}")
