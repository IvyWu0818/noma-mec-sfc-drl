...
from envs.iiot_env_v20 import IIoTEnvV20, TX_POWER_W, NOISE_POWER_W, BANDWIDTH_MHZ
...
class V20MetricsCallback(BaseCallback):
    """
    SAC on V20 env (SAC is now the project's primary DRL algorithm, replacing TD3 —
    see project decision after V18 multi-seed comparison). Same metric set as prior
    versions for continuity with existing plot/eval scripts.
    """
    ...
    def _on_step(self) -> bool:
        ...
        if "cpu_viol_rate"    in info: self._ep_cpu_viol_rates.append(info["cpu_viol_rate"])
        ...
        if self.locals["dones"][0]:
            ...
            self.metrics["episode_cpu_viol_rate"].append(_mean(self._ep_cpu_viol_rates))
...
def main():
    ...
    parser.add_argument("--seed", type=int, default=None,
                         help="SB3 training seed (network init / entropy sampling / replay "
                              "sampling). Omit to reproduce the original unseeded V20 run.")
    args = parser.parse_args()
    suffix = f"_seed{args.seed}" if args.seed is not None else ""

    env    = Monitor(IIoTEnvV20(), f"logs/v20_sac{suffix}_")

    callback = V20MetricsCallback()
    ...
    print("=" * 60)
    print("SAC V20 Training (SAC now primary algorithm; env unchanged from V17-V19)")
    print("=" * 60)
    ...
    model.save(f"models/sac_iiot_v20{suffix}_final")

    output_path = f"results/sac_v20{suffix}_training_metrics.json"
    with open(output_path, "w") as f:
        json.dump(callback.metrics, f, indent=2)
    print(f"SAC V20 data saved to {output_path}")
