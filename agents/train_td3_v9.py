...
class V9MetricsCallback(BaseCallback):
    """
    收集每 episode (100 tasks) 的訓練指標。
    V9 新增：
      - episode_avg_sinr          : 平均 NOMA SINR
      - episode_avg_channel_rate  : 平均有效速率 R_{u,k}
      - episode_channel_overflow_ratio : 子通道超用比例（公式 3 違規率）
      - episode_avg_channel_entropy    : 通道指派熵（均勻度）
    """
    ...
    def _channel_entropy(self, assignments: list) -> float:
        """Shannon entropy of channel assignment distribution (0~log2(K))"""
        if not assignments:
            return 0.0
    ...
    def _on_step(self) -> bool:
        ...
        if self.locals["dones"][0]:
            ...
            overflow_ratio = _mean(self._ep_channel_overflows)
            ch_entropy = self._channel_entropy(self._ep_assigned_chs)
            ...
            self.metrics["episode_channel_overflow_ratio"].append(overflow_ratio)
            self.metrics["episode_avg_channel_entropy"].append(ch_entropy)
            self._reset_ep()
...
def main():
    env = Monitor(IIoTEnvV9(), "logs/v9_")
    n_act = env.action_space.shape[-1]  # 15

    # 略提高 sigma 因 action 維度增加（15 維）
    action_noise = NormalActionNoise(
        mean=np.zeros(n_act),
        sigma=0.14 * np.ones(n_act)
    )

    callback = V9MetricsCallback()
    ...
    print("🚀 V9 訓練啟動")
    print("   obs=18 (含 ch_rem×3), action=15 (含 channel_scores×3)")
    print("   NOMA: K=3 channels, M=2 max/channel, Shannon rate (公式10)")
    print("   Reward: delay + beta*slack + 5*cpu_viol + 0.5*t_comp")
    print("           + 1.5*deadline_pressure + 3.0*channel_overflow")
    print("   Total timesteps: 1,000,000 | Episode length: 100 tasks")
    model.learn(total_timesteps=1_000_000, callback=callback)
    model.save("models/td3_iiot_v9_final")

    output_path = "results/td3_v9_training_metrics.json"
    with open(output_path, "w") as f:
        json.dump(callback.metrics, f, indent=2)
    print(f"✅ V9 數據已存至 {output_path}")
