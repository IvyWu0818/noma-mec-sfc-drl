...
class V8MetricsCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if "t_ul" in info:
            self._ep_t_ul.append(info["t_ul"])
        if "t_comp" in info:
            self._ep_t_comp.append(info["t_comp"])
        if "t_link" in info:
            self._ep_t_link.append(info["t_link"])
        ...
        if self.locals["dones"][0]:
            ...
            self.metrics["episode_avg_deadline_pressure"].append(_mean(self._ep_deadline_pressure))
...
def main():
    env = Monitor(IIoTEnvV8(), "logs/v8_")
    ...
    print("🚀 V8 訓練啟動")
    print("   Multi-regime tasks: urgent / compute-heavy / bandwidth-heavy")
    print("   Reward: delay + beta*slack + 5*cpu_viol + 0.5*t_comp + 1.5*deadline_pressure")
    model.learn(total_timesteps=3000000, callback=callback)
    model.save("models/td3_iiot_v8_final")

    output_path = "results/td3_v8_training_metrics.json"
    with open(output_path, "w") as f:
        json.dump(callback.metrics, f, indent=2)
    print(f"✅ V8 數據已存至 {output_path}")
