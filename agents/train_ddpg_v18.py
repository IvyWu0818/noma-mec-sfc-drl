...
class V18MetricsCallback(BaseCallback):
    """
    DDPG on V18 env: same metric set as TD3 V18, for direct comparison.
    """
...
def main():
    ...
    env    = Monitor(IIoTEnvV18(), f"logs/v18_ddpg{suffix}_")
    ...
    print("=" * 60)
    print("DDPG Training on V18 env (same hyperparams as TD3 V18, for comparison)")
    print("=" * 60)
    ...
    model.save(f"models/ddpg_iiot_v18{suffix}_final")

    output_path = f"results/ddpg_v18{suffix}_training_metrics.json"
    with open(output_path, "w") as f:
        json.dump(callback.metrics, f, indent=2)
    print(f"DDPG V18 data saved to {output_path}")
