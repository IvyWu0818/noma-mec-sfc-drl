...
from envs.iiot_env_v16 import IIoTEnvV16, TX_POWER_W, NOISE_POWER_W, BANDWIDTH_MHZ
...
class V16MetricsCallback(BaseCallback):
    """
    V15: 同 V14 追蹤項目
    """
...
def main():
    env = Monitor(IIoTEnvV16(), "logs/v16_")
    ...
    print("=" * 60)
    print("V16 Training (t_link coeff 1.5 -> 0.8, effective 2.5 -> 1.8)")
    print("=" * 60)
    ...
    print(f"  cost = 1.0*delay + 12.0*slack + 5.0*cpu_viol")
    print(f"       + 0.5*t_comp + 0.8*t_link + 1.5*pressure + 15.0*overflow")
    print(f"[Obs=21, Action=16 | Total timesteps: 1,000,000]")
    print("=" * 60)

    model.learn(total_timesteps=1_000_000, callback=callback)
    model.save("models/td3_iiot_v16_final")

    output_path = "results/td3_v16_training_metrics.json"
    with open(output_path, "w") as f:
        json.dump(callback.metrics, f, indent=2)
    print(f"V16 data saved to {output_path}")
