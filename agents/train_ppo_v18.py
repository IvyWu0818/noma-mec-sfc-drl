...
class V18MetricsCallback(BaseCallback):
    """
    PPO on V18 env: same metric set as TD3/SAC V18, for direct comparison.
    """
    ...
    def _on_step(self) -> bool:
        # PPO uses n_envs=1 vectorized rollout, same single-env indexing as off-policy algos
...
def main():
    ...
    parser.add_argument("--seed", type=int, default=None,
                         help="SB3 training seed (network init / rollout sampling). "
                              "Omit to reproduce the original unseeded V17 run.")
    args = parser.parse_args()
    suffix = f"_seed{args.seed}" if args.seed is not None else ""

    env = Monitor(IIoTEnvV18(), f"logs/v18_ppo{suffix}_")

    callback = V18MetricsCallback()

    # On-policy: no replay buffer/action_noise. net_arch kept at [400,300] to
    # match TD3/SAC/DDPG architecture; other hyperparams are SB3 PPO defaults.
    model = PPO(
        "MlpPolicy",
        env,
        learning_rate=3e-4,
    ...
    print("=" * 60)
    print(f"PPO Training on V18 env, seed={args.seed} (on-policy; SB3 default hyperparams, matched net_arch)")
    print("=" * 60)
    ...
    model.save(f"models/ppo_iiot_v18{suffix}_final")

    output_path = f"results/ppo_v18{suffix}_training_metrics.json"
    with open(output_path, "w") as f:
        json.dump(callback.metrics, f, indent=2)
    print(f"PPO V18 data saved to {output_path}")
