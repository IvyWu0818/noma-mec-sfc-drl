...
from envs.iiot_env_v15 import IIoTEnvV15, TX_POWER_W, NOISE_POWER_W, BANDWIDTH_MHZ
...
class V15MetricsCallback(BaseCallback):
    """
    V15: 同 V14 追蹤項目
    """
...
def main():
    env = Monitor(IIoTEnvV15(), "logs/v15_")
    ...
    print("=" * 60)
    print("V15 Training (+ 1.5 x t_link penalty for VNF co-location)")
    print("=" * 60)
    ...
    print(f"  t_link penalty : +1.5 x t_link (effective coeff 2.5, was 1.0)")
    print(f"[Obs=21, Action=16 | Total timesteps: 1,000,000]")
    print("=" * 60)

    model.learn(total_timesteps=1_000_000, callback=callback)
    model.save("models/td3_iiot_v15_final")

    output_path = "results/td3_v15_training_metrics.json"
    with open(output_path, "w") as f:
        json.dump(callback.metrics, f, indent=2)
    print(f"V15 data saved to {output_path}")
