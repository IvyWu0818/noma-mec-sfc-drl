...
class V17MetricsCallback(BaseCallback):
    """
    PPO on V17 env: same metric set as TD3 V17, for direct comparison.
    """
...
def main():
    env    = Monitor(IIoTEnvV17(), "logs/v17_ppo_")
    ...
    print("=" * 60)
    print("PPO Training on V17 env (on-policy; SB3 default hyperparams, matched net_arch)")
    print("=" * 60)
    ...
    model.save("models/ppo_iiot_v17_final")

    output_path = "results/ppo_v17_training_metrics.json"
    with open(output_path, "w") as f:
        json.dump(callback.metrics, f, indent=2)
    print(f"PPO V17 data saved to {output_path}")
