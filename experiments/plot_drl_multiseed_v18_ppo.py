"""
plot_drl_multiseed_v18_ppo.py
Multi-seed comparison: SAC vs TD3 vs PPO, 3 training seeds each
(orig=V17, seed1/seed2=V18). PPO seed1/seed2 trained via
agents/train_ppo_v18.py specifically for this comparison.

Panels:
  1. Training curves: 9 curves (3 algos x 3 seeds), 100-ep moving avg, grouped by algo colour
...
Run: python -m experiments.plot_drl_multiseed_v18_ppo
"""
...
TRAIN_FILES = {
    ...
    "PPO": {"orig":  "results/ppo_v17_training_metrics.json",
            "seed1": "results/ppo_v18_seed1_training_metrics.json",
            "seed2": "results/ppo_v18_seed2_training_metrics.json"},
}
EVAL_FILE  = "results/drl_multiseed_eval_v18_ppo.json"
OUTPUT_DIR = "results/figures_drl_multiseed_ppo"
...
def main():
    ...
    if not os.path.exists(EVAL_FILE):
        print(f"Missing {EVAL_FILE} -- run experiments.eval_drl_multiseed_v18_ppo first")
        return
    ...
    fig.tight_layout()
    savefig_both(fig, os.path.join(OUTPUT_DIR, "drl_multiseed_ppo_training_curve.png"))
    plt.close(fig)
    print("saved drl_multiseed_ppo_training_curve.png (+ .svg)")

    # ── 2. 6-panel eval overview (bars = mean across seeds, err = std across seeds) ─
    SINGLE_CFG = [
        ("episode_rewards",                "Cumulative Reward (mean +/- std across seeds)", "Reward",           "drl_multiseed_ppo_reward.png"),
        ("episode_avg_delay",              "End-to-End Delay (ms)",                          "Avg Delay (ms)",   "drl_multiseed_ppo_delay.png"),
        ("episode_cpu_viol_rate",          "CPU Violation Rate",                             "Rate (raw / 135)", "drl_multiseed_ppo_cpu_viol.png"),
        ("episode_channel_overflow_ratio", "Channel Overflow Ratio",                         "Ratio",            "drl_multiseed_ppo_ch_overflow.png"),
        ("episode_timeout_ratio",          "Timeout Ratio",                                  "Ratio",            "drl_multiseed_ppo_timeout.png"),
    ]
    ...
    fig.tight_layout()
    savefig_both(fig, os.path.join(OUTPUT_DIR, "drl_multiseed_ppo_delay_decomp.png"))
    plt.close(fig)
    print("saved drl_multiseed_ppo_delay_decomp.png (+ .svg)")
    ...
    fig.suptitle(
        "SAC vs TD3 vs PPO -- Multi-Seed Robustness (3 seeds each)\n"
        "Training curves: 100-ep moving avg  |  "
        f"Eval bars: mean +/- std across 3 seeds ({N_EVAL_EPISODES} eps x 100 tasks each)",
        fontsize=12, fontweight="bold", y=1.01
    )
    savefig_both(fig,
                 os.path.join(OUTPUT_DIR, "drl_multiseed_ppo_v18.png"),
                 bbox_inches="tight")
    plt.close(fig)
    print(f"saved drl_multiseed_ppo_v18.png (+ .svg)")
