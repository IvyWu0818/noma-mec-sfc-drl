"""
eval_drl_multiseed_v18_ppo.py
Evaluate SAC / TD3 / PPO across 3 training seeds each:
  - orig  : V17 original unseeded run
  - seed1 : V18 --seed 1
  - seed2 : V18 --seed 2

Same deterministic 20-episode x 100-task eval (seeds 0-19) as
eval_drl_multiseed_v18.py, extended to include PPO (trained via
agents/train_ppo_v18.py, added specifically for this cross-seed comparison).

Run: python -m experiments.eval_drl_multiseed_v18_ppo
"""
...
NUM_TASKS       = 100
N_EVAL_EPISODES = 20
OUTPUT_FILE     = "results/drl_multiseed_eval_v18_ppo.json"
...
MODELS = {
    ...
    "PPO": (PPO, {"orig":  "models/ppo_iiot_v17_final",
                  "seed1": "models/ppo_iiot_v18_seed1_final",
                  "seed2": "models/ppo_iiot_v18_seed2_final"}),
}
...
def main():
    ...
    for algo, (cls, paths) in MODELS.items():
        for s_label, path in paths.items():
            ...
            mean_r = np.mean(results[algo][s_label]["episode_rewards"])
