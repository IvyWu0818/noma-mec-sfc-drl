...
INPUT_FILE = "results/td3_v12_training_metrics.json"
OUTPUT_DIR = "results/figures_v12"
...
def main():
    if not os.path.exists(INPUT_FILE):
        print(f"Cannot find {INPUT_FILE} — run train_td3_v12.py first")
        return
    ...
    single_cfg = [
        ("episode_rewards",               "Cumulative Reward",             "v12_reward.png",            "steelblue"),
        ("episode_avg_delay",             "Avg Delay (ms)",                "v12_delay.png",             "orangered"),
        ("episode_avg_slack",             "Avg Slack (ms)",                "v12_slack.png",             "#2CA02C"),
        ("episode_timeout_ratio",         "Timeout Ratio",                 "v12_timeout.png",           "black"),
        ("episode_avg_cpu_viol",          "Avg CPU Violation",             "v12_cpu_viol.png",          "purple"),
        ("episode_avg_deadline_pressure", "Deadline Pressure",             "v12_deadline_pressure.png", "darkgreen"),
        ("episode_task_mix_urgent_ratio", "Urgent Task Ratio",             "v12_urgent_ratio.png",      "#D62728"),
        ("episode_avg_sinr_db",           "Avg SINR (dB)",                 "v12_sinr.png",              "#1A6FA8"),
        ("episode_avg_channel_rate",      "Avg Channel Rate R (Mbps)",     "v12_channel_rate.png",      "#0F6E56"),
        ("episode_channel_overflow_ratio","Channel Overflow Ratio",        "v12_ch_overflow.png",       "#D85A30"),
        ("episode_avg_channel_entropy",   "Channel Assignment Entropy",    "v12_ch_entropy.png",        "#7F77DD"),
        ("episode_avg_rho",               "Avg Offload Ratio rho",         "v12_rho.png",               "#B8860B"),
        ("episode_avg_queue_delta",       "Avg Queue Delta |dq|",          "v12_queue_delta.png",       "#8B4513"),
    ]
    ...
    for key, ylabel, fname, color in single_cfg:
        ...
        ax.set_title(f"V12 Training -- {ylabel}", fontsize=13)
    ...
    if all(k in data for k in decomp_keys):
        ...
        for key, label, color in zip(decomp_keys, decomp_labels, decomp_colors):
            ...
            bottoms += vals
        ax.set_title("V12 Training -- Delay Decomposition (smoothed)", fontsize=13)
        ax.set_xlabel("Episode")
        ax.set_ylabel("Avg Delay (ms)")
        ax.legend(loc="upper right", fontsize=10)
        ax.grid(True, linestyle="--", alpha=0.4)
        fig.tight_layout()
        fig.savefig(os.path.join(OUTPUT_DIR, "v12_delay_decomp.png"), dpi=300)
        plt.close(fig)
        print("saved v12_delay_decomp.png")
    ...
    for idx, (key, ylabel, color) in enumerate(noma_cfg):
        ...
        ax.grid(True, linestyle="--", alpha=0.4)
    fig.suptitle("TD3 V12 -- NOMA Channel Metrics (5G NR / SIC)", fontsize=13,
                 fontweight="bold", y=1.02)
    fig.savefig(os.path.join(OUTPUT_DIR, "v12_noma_panel.png"),
                dpi=300, bbox_inches="tight")
    plt.close(fig)
    print("saved v12_noma_panel.png")
    ...
    for idx, (key, ylabel, color) in enumerate(panel_cfg):
        ...
        ax.grid(True, linestyle="--", alpha=0.4)
    fig.suptitle("TD3 V12 Training Metrics", fontsize=14, fontweight="bold", y=1.01)
    fig.savefig(os.path.join(OUTPUT_DIR, "v12_panel.png"),
                dpi=300, bbox_inches="tight")
    plt.close(fig)
    print("saved v12_panel.png")

    # ── 5. V10 / V11 / V12 comparison ────────────────────────────────────
    compare_cfg = [
    ...
    loaded = {}
    for label, fpath in [("V10", V10_FILE), ("V11", V11_FILE)]:
        if os.path.exists(fpath):
            with open(fpath) as f:
                loaded[label] = json.load(f)

    colors_ver = {"V10": "lightcoral", "V11": "steelblue", "V12": "seagreen"}
    styles_ver = {"V10": "--",          "V11": "-.",         "V12": "-"}
    ...
    for idx, (key, ylabel) in enumerate(compare_cfg):
        ...
        ax.plot(eps, smooth(data[key]), color=colors_ver["V12"],
                linewidth=2.2, linestyle=styles_ver["V12"], label="V12")
        ax.set_title(ylabel, fontsize=10)
        ax.set_xlabel("Episode", fontsize=8)
        ax.set_ylabel(ylabel, fontsize=8)
        ax.legend(fontsize=9)
        ax.grid(True, linestyle="--", alpha=0.4)
    fig.suptitle("V10 vs V11 vs V12 -- Training Comparison",
                 fontsize=14, fontweight="bold", y=1.01)
    fig.savefig(os.path.join(OUTPUT_DIR, "v10_v11_v12_compare.png"),
                dpi=300, bbox_inches="tight")
    plt.close(fig)
    print("saved v10_v11_v12_compare.png")

    # ── 6. V12-only new metrics panel (slack + urgent ratio) ─────────────
    new_cfg = [
        ("episode_avg_slack",             "Avg Slack (ms)",     "#2CA02C"),
        ("episode_task_mix_urgent_ratio", "Urgent Task Ratio",  "#D62728"),
    ]
    if all(k in data for k, *_ in new_cfg):
        fig, axes = plt.subplots(1, 2, figsize=(14, 5))
        for ax, (key, ylabel, color) in zip(axes, new_cfg):
            ax.plot(eps, data[key], color="lightgray", alpha=0.35, linewidth=0.8, label="raw")
            ax.plot(eps, smooth(data[key]), color=color, linewidth=2.0, label="smoothed (w=15)")
            ax.set_title(f"V12 -- {ylabel}", fontsize=12)
            ax.set_xlabel("Episode")
            ax.set_ylabel(ylabel)
            ax.legend(fontsize=10)
            ax.grid(True, linestyle="--", alpha=0.5)
        fig.suptitle("TD3 V12 -- New Metrics", fontsize=13, fontweight="bold")
        fig.tight_layout()
        fig.savefig(os.path.join(OUTPUT_DIR, "v12_new_metrics.png"), dpi=300)
        plt.close(fig)
        print("saved v12_new_metrics.png")

    # ── 7. Numeric summary (last 20 episodes) ────────────────────────────
    print("\nV12 Training Summary (last 20 episodes mean +/- std):")
    report = [
