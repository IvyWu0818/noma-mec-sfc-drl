...
INPUT_FILE = "results/td3_v8_training_metrics.json"
OUTPUT_DIR = "results/figures_v8"
...
def main():
    if not os.path.exists(INPUT_FILE):
        print(f"❌ 找不到 {INPUT_FILE}，請先執行 train_td3_v8.py")
        return
    ...
    single_cfg = [
        ("episode_rewards", "Cumulative Reward", "v8_reward.png", "steelblue"),
        ("episode_avg_delay", "Avg Delay (ms)", "v8_delay.png", "orangered"),
        ("episode_timeout_ratio", "Timeout Ratio", "v8_timeout.png", "black"),
        ("episode_avg_cpu_viol", "Avg CPU Violation", "v8_cpu_viol.png", "purple"),
        ("episode_avg_deadline_pressure", "Deadline Pressure", "v8_deadline_pressure.png", "darkgreen"),
        ("episode_task_mix_urgent_ratio", "Urgent Task Ratio", "v8_urgent_ratio.png", "brown"),
    ]
    ...
    for key, ylabel, fname, color in single_cfg:
        ...
        ax.set_title(f"V8 Training — {ylabel}", fontsize=13)
    ...
    if all(k in data for k in decomp_keys):
        ...
        for key, label, color in zip(decomp_keys, decomp_labels, decomp_colors):
            ...
            bottoms += vals
        ax.set_title("V8 Training — Delay Decomposition (smoothed)", fontsize=13)
        ax.set_xlabel("Episode")
        ax.set_ylabel("Avg Delay (ms)")
        ax.legend(loc="upper right", fontsize=10)
        ax.grid(True, linestyle="--", alpha=0.4)
        fig.tight_layout()
        fig.savefig(os.path.join(OUTPUT_DIR, "v8_delay_decomp.png"), dpi=300)
        plt.close(fig)
        print("✅ v8_delay_decomp.png")
    ...
    panel_cfg = [
        ...
        ("episode_avg_deadline_pressure", "Deadline Pressure", "darkgreen"),
    ]
    ...
    fig.suptitle("TD3 V8 Training Metrics", fontsize=14, fontweight="bold", y=1.01)
    fig.savefig(os.path.join(OUTPUT_DIR, "v8_panel.png"), dpi=300, bbox_inches="tight")
    plt.close(fig)
    print("✅ v8_panel.png")

    print("\n📊 V8 訓練摘要 (最後 20 episodes 平均)：")
    report = [
