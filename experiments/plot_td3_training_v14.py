...
INPUT_FILE = "results/td3_v14_training_metrics.json"
OUTPUT_DIR = "results/figures_v14"
...
def main():
    if not os.path.exists(INPUT_FILE):
        print(f"Cannot find {INPUT_FILE} -- run train_td3_v14.py first")
        return
    ...
    single_cfg = [
        ("episode_rewards",               "Cumulative Reward",             "v14_reward.png",            "steelblue"),
        ("episode_avg_delay",             "Avg Delay (ms)",                "v14_delay.png",             "orangered"),
        ("episode_avg_slack",             "Avg Slack (ms)",                "v14_slack.png",             "#2CA02C"),
        ("episode_timeout_ratio",         "Timeout Ratio",                 "v14_timeout.png",           "black"),
        ("episode_avg_cpu_viol",          "Avg CPU Violation",             "v14_cpu_viol.png",          "purple"),
        ("episode_avg_deadline_pressure", "Deadline Pressure",             "v14_deadline_pressure.png", "darkgreen"),
        ("episode_task_mix_urgent_ratio", "Urgent Task Ratio",             "v14_urgent_ratio.png",      "#D62728"),
        ("episode_avg_sinr_db",           "Avg SINR (dB)",                 "v14_sinr.png",              "#1A6FA8"),
        ("episode_avg_channel_rate",      "Avg Channel Rate R (Mbps)",     "v14_channel_rate.png",      "#0F6E56"),
        ("episode_channel_overflow_ratio","Channel Overflow Ratio",        "v14_ch_overflow.png",       "#D85A30"),
        ("episode_avg_channel_entropy",   "Channel Assignment Entropy",    "v14_ch_entropy.png",        "#7F77DD"),
        ("episode_avg_rho",               "Avg Offload Ratio rho",         "v14_rho.png",               "#B8860B"),
        ("episode_avg_queue_delta",       "Avg Queue Delta |dq|",          "v14_queue_delta.png",       "#8B4513"),
    ]
    ...
    for key, ylabel, fname, color in single_cfg:
        ...
        ax.set_title(f"V14 Training -- {ylabel}", fontsize=13)
    ...
    if all(k in data for k in decomp_keys):
        ...
        for key, label, color in zip(decomp_keys, decomp_labels, decomp_colors):
            ...
            bottoms += vals
        ax.set_title("V14 Training -- Delay Decomposition (smoothed)", fontsize=13)
        ax.set_xlabel("Episode")
        ax.set_ylabel("Avg Delay (ms)")
        ax.legend(loc="upper right", fontsize=10)
        ax.grid(True, linestyle="--", alpha=0.4)
        fig.tight_layout()
        fig.savefig(os.path.join(OUTPUT_DIR, "v14_delay_decomp.png"), dpi=300)
        plt.close(fig)
        print("saved v14_delay_decomp.png")
    ...
    for idx, (key, ylabel, color) in enumerate(noma_cfg):
        ...
        ax.grid(True, linestyle="--", alpha=0.4)
    fig.suptitle("TD3 V14 -- NOMA Channel Metrics (overflow penalty=15 + SIC degradation)",
                 fontsize=13, fontweight="bold", y=1.02)
    fig.savefig(os.path.join(OUTPUT_DIR, "v14_noma_panel.png"),
                dpi=300, bbox_inches="tight")
    plt.close(fig)
    print("saved v14_noma_panel.png")
    ...
    for idx, (key, ylabel, color) in enumerate(panel_cfg):
        ...
        ax.grid(True, linestyle="--", alpha=0.4)
    fig.suptitle("TD3 V14 Training Metrics", fontsize=14, fontweight="bold", y=1.01)
    fig.savefig(os.path.join(OUTPUT_DIR, "v14_panel.png"),
                dpi=300, bbox_inches="tight")
    plt.close(fig)
    print("saved v14_panel.png")

    # ── 5. V11 / V13 / V14 comparison ────────────────────────────────────
    compare_cfg = [
    ...
    loaded = {}
    for label, fpath in [("V11", V11_FILE), ("V13", V13_FILE)]:
        if os.path.exists(fpath):
            with open(fpath) as f:
                loaded[label] = json.load(f)

    colors_ver = {"V11": "lightcoral", "V13": "steelblue", "V14": "seagreen"}
    styles_ver = {"V11": "--",          "V13": "-.",         "V14": "-"}
    ...
    for idx, (key, ylabel) in enumerate(compare_cfg):
        ...
        ax.plot(eps, smooth(data[key]), color=colors_ver["V14"],
                linewidth=2.2, linestyle=styles_ver["V14"], label="V14")
        ax.set_title(ylabel, fontsize=10)
        ax.set_xlabel("Episode", fontsize=8)
        ax.set_ylabel(ylabel, fontsize=8)
        ax.legend(fontsize=9)
        ax.grid(True, linestyle="--", alpha=0.4)
    fig.suptitle("V11 vs V13 vs V14 -- Training Comparison",
                 fontsize=14, fontweight="bold", y=1.01)
    fig.savefig(os.path.join(OUTPUT_DIR, "v11_v13_v14_compare.png"),
                dpi=300, bbox_inches="tight")
    plt.close(fig)
    print("saved v11_v13_v14_compare.png")

    # ── 6. Numeric summary (last 20 episodes) ────────────────────────────
    print("\nV14 Training Summary (last 20 episodes mean +/- std):")
    report = [
