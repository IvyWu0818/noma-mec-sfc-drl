...
class IIoTEnvV16(gym.Env):
    """
    V16: 調整 t_link 懲罰係數，尋找 t_link 與 overflow/CPU_viol 的平衡點

    ═══════════════════════════════════════════════════════════════════
    V15 → V16 的改變
    ═══════════════════════════════════════════════════════════════════

    [調整] t_link 懲罰係數 1.5 → 0.8（有效係數 2.5 → 1.8）
      V15 問題: 係數 1.5 過強，agent 過度集中 VNF 到同一節點
               導致 CPU Viol 0.47（算力超載），overflow 回升到 14.9%
      修法: 降至 0.8，仍比 V14 的純 delay 路徑（係數 1.0）多懲罰 t_link，
            但不像 V15 那麼強勢，讓 agent 在 VNF 放置與 overflow 之間
            能找到更平衡的策略

    完整 cost 函數（V16）:
      cost = 1.0×delay + 12.0×slack + 5.0×cpu_viol
           + 0.5×t_comp + 0.8×t_link + 1.5×deadline_pressure
           + 15.0×channel_overflow
      t_link 有效係數: 1.0（via delay） + 0.8（explicit） = 1.8

    保留 V15 全部其他設定:
      overflow penalty 15.0, ru_k×0.25 on overflow
      queue_load_norm [0,1], 5G NR 參數
    ...
    def step(self, action):
        ...
        cost = (
            ...
            + 5.0       * cpu_viol
            + 0.5       * t_comp
            + 0.8       * t_link             # V16: 有效係數 1.8（V15 的 2.5 → 降低）
