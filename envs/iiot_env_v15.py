...
class IIoTEnvV15(gym.Env):
    """
    V15: 修正 t_link 過高問題，加入 backhaul 延遲顯式懲罰

    ═══════════════════════════════════════════════════════════════════
    V14 → V15 的改變
    ═══════════════════════════════════════════════════════════════════

    [改動] cost 加入 + 1.5 × t_link
      V14 問題: t_link = 3.38ms，佔總延遲 49%，卻只透過 1.0×delay 被懲罰
      修法: 顯式加入 1.5×t_link，使 t_link 有效權重從 1.0 升至 2.5
            等同 t_comp 的懲罰力道 (0.5+1.0=1.5 → t_link 變 2.5)
      預期: agent 更傾向把多個 VNF 放在同一 MEC 節點（t_link=0）
            而非分散到不同節點（t_link 最高可達 5ms）
    ...
    def step(self, action):
        ...
        cost = (
            ...
            + 5.0       * cpu_viol
            + 0.5       * t_comp
            + 1.5       * t_link             # V15: 顯式 backhaul 懲罰（有效係數 2.5）
