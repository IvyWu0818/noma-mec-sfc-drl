...
class IIoTEnvV14(gym.Env):
    """
    V14: 修正 Channel Overflow 訓練訊號，統一 Obs 量綱

    ═══════════════════════════════════════════════════════════════════
    V13 → V14 的改變
    ═══════════════════════════════════════════════════════════════════

    [改動 1] Overflow 懲罰加強：5.0 → 15.0
      + Overflow 時 ru_k × 0.25，模擬第三用戶加入通道後 SIC 失敗
      導致吞吐率大幅下降，讓 agent 感受到 overflow 的延遲代價，
      而不只是固定扣分（原來 0.1 reward 太弱，agent 選擇忽略）。

    [改動 2] Obs queue_load_abs 改為正規化 [0,1]
      原: queue_load_abs = raw MCycles/ms，值域 0–55
      新: queue_load_norm = queue_load / cpu_capacity，值域 [0,1]
      理由: 與 mec_rem、ch_rem 量綱一致（皆 [0,1]），
            避免大尺度的 queue_load 數值淹沒 ch_rem 的梯度訊號。

    保留 V13 全部設定:
      5G NR 參數 (TX_POWER_W=0.2, NOISE_POWER_W=4e-3, TIME_SLOT_MS=0.5)
      Obs[14] = max(|h|²)（V11 原始寫法）

    ═══════════════════════════════════════════════════════════════════
    Obs 維度 (21)：同 V13，但 queue_load_abs → queue_load_norm [0,1]
    Action 維度 (16)：同 V13
    ═══════════════════════════════════════════════════════════════════
    """
    ...
    def _get_obs(self):
        ...
        # V14: 正規化成 [0,1]，與 mec_rem / ch_rem 量綱一致
        queue_load_norm = [
        ...
        return np.array([
            ...
            *queue_load_norm,               # 各 MEC 佇列負載正規化 [0,1] ×3  (V14)
            total_c,                        # 任務總計算量 MCycles
            float(pressure),                # 壓力指標 (data+cpu)/deadline
            best_sinr,                      # 最佳通道功率增益 max(|h|²)
    ...
    def step(self, action):
        ...
        # V14: overflow 時模擬 SIC 失敗 → rate 降為 1/4
        if channel_overflow:
            ru_k *= 0.25
        ...
        cost = (
            ...
            + 15.0      * channel_overflow   # V14: 5.0 → 15.0
        )
