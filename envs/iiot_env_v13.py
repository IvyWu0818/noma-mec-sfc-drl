...
class IIoTEnvV13(gym.Env):
    """
    V13: 保留 V12 全部 5G NR 參數，還原 V11 的通道增益 obs 寫法

    ═══════════════════════════════════════════════════════════════════
    V12 → V13 的改變
    ═══════════════════════════════════════════════════════════════════

    [還原] Obs 中的通道品質欄位改回 V11 寫法
      V12: spectral_eff = log₂(1 + P×max(|h|²)/N)  範圍 ~3–7 bps/Hz
      V13: best_sinr    = max(|h|²)                 範圍 ~0.1–2.0
      原因: V12 的頻譜效率表示讓 agent 無法從 obs 學到 rho 與通道的關係，
            導致 rho 收斂到 0.97（幾乎全部頻寬）而非 V11 的 0.16，
            進而使 t_ul 與 timeout 大幅增加。

    保留 V12 的所有其他 5G NR 修正:
      TX_POWER_W = 0.2 W (23 dBm, 3GPP PC3)
      NOISE_POWER_W = 4e-3 (SNR~17 dB)
      TIME_SLOT_MS = 0.5 ms (5G NR numerology µ=1)
      BACKHAUL_DELAY_MS, |h|² ~ Exp(1) 功率增益標記

    ═══════════════════════════════════════════════════════════════════
    Obs 維度 (21)：同 V11/V12，sinr obs = max(|h|²)（V11 原始寫法）
    Action 維度 (16)：同 V11/V12
    ═══════════════════════════════════════════════════════════════════
    """
    ...
    def _get_obs(self):
        ...
        # V13 還原 V11 寫法: 直接回傳最佳通道的功率增益 max(|h|²)，範圍 ~0.1–2.0
        ...
        return np.array([
            ...
            *queue_load_abs,                # 各 MEC 佇列負載 MCycles/ms ×3
            ...
            best_sinr,                      # 最佳通道功率增益 max(|h|²)（V11 原始寫法）
